...
const ProviderIcon = ({ src, alt, size, className, fallbackText }: any) => (
  <div className={`relative flex items-center justify-center overflow-hidden bg-black/20 ${className || ""}`}>
    {src ? (
      ...
      <span className="text-[10px] font-bold text-white uppercase">{fallbackText}</span>
    )}
  </div>
);
...
export default function FlowAnimation() {
  ...
  return (
    <div className="mt-16 w-full max-w-4xl relative h-[360px] hidden md:flex items-center justify-center animate-[float_6s_ease-in-out_infinite]">
      ...
      <div className="relative z-20 w-32 h-32 rounded-full bg-[#23180f] border-2 border-[#f97815] shadow-[0_0_40px_rgba(249,120,21,0.3)] flex flex-col items-center justify-center gap-1 group cursor-pointer hover:scale-105 transition-transform duration-500">
        <div className="flex flex-col items-center justify-center gap-1">
          <div className="size-9 rounded-lg bg-linear-to-br from-[#f97815] to-orange-700 flex items-center justify-center text-white shadow-lg overflow-hidden border border-white/10">
            <Image src="/icons/logo.jpg" alt="Logo" width={40} height={40} className="w-full h-full object-cover" />
          </div>
        </div>
      ...
      {/* SVG Lines from Tinobot to Providers */}
      <svg
      ...
      >
        <path
          ...
          className={
            activeFlow === 0
              ? "stroke-[#f97815] [stroke-width:3] animate-pulse"
              : "stroke-gray-600 [stroke-width:2]"
          }
        ></path>
        <path
